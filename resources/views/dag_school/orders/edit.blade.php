            ...
            <table id="tbl_main" class="dataTable compact row-border hover" style="width: 100%" > 
                <thead>
                    <tr>
                        ...
                        <th class="col_qty" style="text-align: center; width: 50%;">จำนวนสั่ง</th>
                ...
                <tbody>  
                      ...
                      <tr @if($item->product_category_id==1) class="table-success" @else class="table-warning" @endif /> 
                        ...
                        <td style="color:{{(($key%2==1)?' green ':  ' blue ')}};">
                          ...
                          @if($order->status<5)
                            @if($item->order_qty != 0)
                              <input type="number" min="0" step="0.1" name="qtys[]"  class="form-control" style="text-align: right; width: 50px;" value="{{ $item->order_qty }}" />
                            @else
                              <input type="number" min="0" step="0.1" name="qtys[]"  class="form-control" style="text-align: right; width: 50px;" value="0" />
                            @endif 
                          @else
                        ...
                        @if(Auth::user()->isOnlineStoreCustomer() && $order->status!= 5)
                        <td>
                          <a href="#" name="btn_item_del" class="btn btn-danger" data-product_name="{{$item->product_name}}" data-id="{{$item->id}}" /><i class="fa fa-trash"></i> ลบ</a>
                        </td>
                        @endif
                      </tr>
                      @endforeach
                ...
                <tbody> 
                      ...
                      <tr @if($item->product_category_id==1) class="table-success" @else class="table-warning" @endif /> 
                        ...
                        <td style="color:{{(($key%2==1)?' green ':  ' blue ')}}">
                          ...
                          @if($order->status<5)
                            @if($item->order_qty != 0)
                              <input type="number" min="0" step="0.1" name="more_qtys[]"  class="form-control" style="text-align: right; width: 50px;" value="{{ $item->order_qty }}" />
                            @else
                              <input type="number" min="0" step="0.1" name="more_qtys[]"  class="form-control" style="text-align: right; width: 50px;" value="0" />
                            @endif 
                          @else
                            <span>{{ $item->order_qty }}</span>
                          @endif
  ...
  $('#frm1').submit(function(e){ 
    ...
    $('a[name="btn_item_del"]').click(function(e){       
      ...
      if(confirm("คุณต้องการที่จะลบรายการ\n\n"+ele_temp.attr('data-product_name')+"\n\nและนำออกจากรายการสั่งสินค้าตั้งต้น ใช่หรือไม่ ? ")){
        ...
        $.ajax({
          url: "{{ url('/online_store/orders/remove-item') }}/"+ele_temp.attr('data-id'),
          type: 'post', dataType:"json", data: params,
          success: function (res) { 
            // console.log(res);
            if(res.success=="success"){
