...
<section>
  <div class="container-fluid">
    ...
    <div class="row">
      <div class="col-md-12">
        หลักสูตร : 
        <select name="program_student_name" class="form-control">
          <option value=""> - Select - </option>
          @foreach($program_student_headers as $val)
            <option value="{{$val->id}}">{{$val->program_student_name}}</option>
          @endforeach
        </select>
        <input type="hidden" name="program_student_id" value="" />
      </div>
    </div>
...
$(document).ready(function(){
        ...
        if ($.fn.DataTable.isDataTable('#tbl_main')) {
            ...
            tbl_main = $('#tbl_main').DataTable( {
                ...
                ajax: {
                    ...
                    dataType:"json",
                    data: {
                      program_student_id: function() { return $('input[name="program_student_id"]').val() },
                    },
                    dataSrc: 'items',
                },
                autoWidth: false,
    ...
    $('select[name="program_student_name"]').on('change', function() {
      alert($(this).val());
      $('input[name="program_student_id"]').val($(this).val());
      getList();
    });
