...
<div class="breadcrumb-holder">
  <div class="container-fluid">
    <ul class="breadcrumb">
      ...
      <li class="breadcrumb-item active">แบบทดสอบ โดยวิชาของหลักสูตร</li>
    </ul>
  </div>
</div>
<!-- Breadcrumb-->
    ...
    function getList(){
        if ($.fn.DataTable.isDataTable('#tbl_main')) {
            ...
            tbl_main = $('#tbl_main').DataTable( {
                ...
                ajax: {
                    type: 'GET',
                    url: "{{ url('dag_school/program-course-tests/list') }}",
                    data: {
                    ...
                    {   targets: 'col_no',
                        ...
                        className: 'dt-center',
                         render: function (data, type, row, meta) {
                            // var tmp = meta.row+1;
                            var tmp = (row.course_hierarchy.substring(3,9)=='000000'?'<b>'+row.course_no+'</b>':row.course_no);
                            return tmp;
                         }
                     }, 
                    {   targets: 'col_name',
                        ...
                        className: 'dt-center',
                         render: function (data, type, row, meta) {
                            // var tmp = meta.row+1;
                            var tmp = (row.course_hierarchy.substring(3,9)=='000000'?'<b>'+row.course_name+'</b>':row.course_name);
                            return tmp;
                         }
                     }, 
                    {   targets: 'col_test',
                        ...
                        className: 'dt-center',
                         render: function (data, type, row, meta) {
                            // var tmp = meta.row+1;
                            var tmp = (row.course_hierarchy.substring(3,9)=='000000'?'<b>'+row.program_course_test_name+'</b>':row.program_course_test_name);
                            return tmp;
                         }
                     }, 
                    {   targets: 'col_score',
