...
<section>
  <div class="container-fluid">
    ...
    <!-- Page Header-->
    <header> 
      <h1 class="display">รายการกลุ่มหลักวิชา
	  &nbsp;<a href="{{ url('dag_school/main_courses/new') }}" class="btn btn-primary shadow rounded" name="btn_add" /> เพิ่ม</a>
      </h1>
	    ...
	    <table id="tbl_main" class="hover compact" style="width: 100%;"   >
	        <thead>
	            <tr>
                  ...
                  <th class="col_name" style="text-align: center;" >ชื่อกลุ่มหลักวิชา</th>
    ...
    function getList(){
        if ($.fn.DataTable.isDataTable('#tbl_main')) {
            ...
            tbl_main = $('#tbl_main').DataTable( {
                ...
                ajax: {
                    type: 'GET',
                    url: "{{ url('dag_school/main_courses/list') }}",
                    dataSrc: 'items',
                },
                autoWidth: false,
                columns: [
                ...
                columnDefs: [
                    ...
                    {   targets: 'col_action',
                        className: 'dt-center',
                         render: function (data, type, row, meta) {
                          var tmp = '<a name="" href="{{ url('dags/main_courses/edit/') }}/'+row.id+'" class="btn btn-primary btn-mini rounded" data-ref_id="'+row.id+'" href="#" ><i class="fa fa-edit"></i> แก้ไข</a>';
                            return tmp;
                         }
                     }
                ]
