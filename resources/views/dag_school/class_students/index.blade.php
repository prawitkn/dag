...
<div class="breadcrumb-holder">
  <div class="container-fluid">
    <ul class="breadcrumb">
      ...
      <li class="breadcrumb-item active">นักเรียน โดยรุ่นหลักสูตร</li>
    </ul>
  </div>
</div>
<!-- Breadcrumb-->
      ...
      <div class="col-md-12">
        <select name="program_class_name" class="form-control">
          ...
          @foreach($program_classes as $val)
            <option value="{{$val->id}}">{{$val->program_name }} : {{$val->program_class_name}}</option>
          @endforeach
        </select>
...
$(document).ready(function(){
    ...
    function getList(){
        if ($.fn.DataTable.isDataTable('#tbl_main')) {
            ...
            tbl_main = $('#tbl_main').DataTable( {
                ...
                ajax: {
                    type: 'GET',
                    url: "{{ url('dag_school/class-students/list') }}",
                    data: {
    ...
    $('select[name="program_class_name"]').on('change', function() { 
      // alert($(this).val());
      $('input[name="program_class_id"]').val($(this).val());
      getList();
    });
