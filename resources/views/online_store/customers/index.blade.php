	    ...
	    <table id="tbl_main" class="hover compact" style="width: 100%;"   >
	        <thead>
	            <tr>
                  ...
                  <th class="col_name" style="text-align: center;" >ชื่อลูกค้า</th>
    ...
    function getList(){
        if ($.fn.DataTable.isDataTable('#tbl_main')) {
            ...
            tbl_main = $('#tbl_main').DataTable( {
                ...
                ajax: {
                    type: 'GET',
                    url: "{{ url('online_store/customers/list') }}",
                    dataSrc: 'items',
                },
                autoWidth: false,
                columns: [
                        ...
                        className: 'dt-center',
                         render: function (data, type, row, meta) {
                          var tmp = '<a name="" href="{{ url('online_store/customers/edit/') }}/'+row.id+'" class="btn btn-primary btn-mini rounded" data-ref_id="'+row.id+'" href="#" ><i class="fa fa-edit"></i> แก้ไข</a> '+
                            '<a name="" target="_blank" href="{{ url('online_store/customers/edit-product-list/') }}/'+row.id+'" class="btn btn-primary btn-mini rounded" data-ref_id="'+row.id+'" href="#" ><i class="fa fa-edit"></i> สินค้าสั่งประจำ</a> '+
                            '<a name="" target="_blank" href="{{ url('online_store/ship_to_customers/view-list/') }}/'+row.id+'" class="btn btn-primary btn-mini rounded" data-ref_id="'+row.id+'" href="#" ><i class="fa fa-edit"></i> สาขา</a>';
                            return tmp;
                         }
                     }
                ]
